...
%\section{The problem}
    ...
    The problem consists in the planning of the maintenance operations for military aircraft and the assignment of these aircraft to already scheduled missions. In order to generalize the formulation, from now on aircraft will be named "resources" and missions will be named "tasks". A series of $j \in \mathcal{J}$ tasks are planned along a horizon divided into $t \in \mathcal{T}$ periods. Each task requires a certain number $rr_{j}$ of resources $r \in \mathcal{R}$ which it employs for a time duration defined by $rh_j$ in each period. Set $cd_{j} \subset \mathcal{R}$ lists the resources that can be assigned to each task.
...
\section{Mathematical model}
    ...
    % $rh_{vt}$        & : & time consumed of any resource when assigned to task $j$ in period $t$. \\
    % $rr_{vt}$        & : & number or resources to assign to task $j$ in period $t$. \\            
    ...
    % $rut\_i_a$       & : & remaining used time for resource $r$ at the beginning of horizon. \\        
    % $ret\_i_a$       & : & remaining elapsed time for resource $r$ at the beginning of horizon. \\
    % $state\_i_{rt}$  & : & some $s \in \mathcal{S}$ in case the resource $r$ has a fixed state. \\
    % \end{tabular}
    % \bigskip

    % \subsection{Variables}

    The decision variables manage the previously defined resources. We can summarize the main decisions the following way: if resource $r$ is in state $s$ in each period $t$ and if resource $r$ is assigned to task $j$ in period $t$. We call $\mathcal{T}0$ to periods that include the previous period, $0$.

    \begin{tabular}{p{15mm}lp{105mm}}
        $task_{rjt}$        & : & 1 if resource $r \in cd_j$ is assigned to task $j$ in period $t$, 0 otherwise. \\  
        $state_{rst}$       & : & 1 if resource $r$ is in state $s$ in period $t$, 0 otherwise. \\  
        $start_{rt}$        & : & 1 if resource $r$ starts maintenance in period $t$, 0 otherwise. \\
        $rut_{rt}$          & : & remaining usage time (integer) for resource $r$ after period $t \in \mathcal{T}0$. \\  
        $ret_{rt}$          & : & remaining elapsed time (integer) for resource $r$ after period $t \in \mathcal{T}0$. \\  
        $used_{rt}$         & : & time (integer) used by resource $r$ in period $t$. \\
    ...
    \begin{align}
        ...
        & \sum_{s \in \mathcal{S}} state_{rst} = 1 
                & \hspace{10mm} t \in \mathcal{T}, r \in \mathcal{R} \label{eq:state2}
    \end{align}
    ...
    Tasks' resource requirements are defined by \ref{eq:taskres}, maintenance capacity is controlled by \ref{eq:capacity}. Constraints \ref{eq:state1} and \ref{eq:state3} tie states with the assignment to a task. Maximum tasks and states assigned to each resource is guaranteed by \ref{eq:maxtasks} to \ref{eq:state2}.
    ...
    \begin{align}
        ...
        % & used_{rt} \geq \sum_{j \in \mathcal{J}} task_{rjt} \times rh_{j}
        %         & r \in \mathcal{R}, t \in \mathcal{T} \label{eq:usage}\\
        % definition of consumed resource hours (bigger):
        & used_{rt} \geq task_{rjt} \times rh_{j}
                & r \in \mathcal{R}, t \in \mathcal{T}, j \in \mathcal{J} \label{eq:usage2}\\
        % remaining used time
        & rut_{rt} \leq (rut_{rt-1} - used_{rt}) + (mu \times start_{rt})
                & t \in \mathcal{T}, r \in \mathcal{R} \label{eq:balance1}\\
        % remaining elapsed time. TODO: improve this constraint like the maintenance state duration
        & ret_{rt} \leq (ret_{rt-1} - 1) + (me \times start_{rt})
                & t \in \mathcal{T}, r \in \mathcal{R} \label{eq:balance2}\\
        & rut_{rt} = rutInit_r
                & t = 0, r \in \mathcal{R} \label{eq:initial}\\
        & ret_{rt} = retInit_r
                & t = 0, r \in \mathcal{R} \label{eq:initial2}\\
        & ret_{rt} \leq me
                & t \in \mathcal{T}, r \in \mathcal{R} \label{eq:me}\\
        & rut_{rt} \leq mu
                & t \in \mathcal{T}, r \in \mathcal{R} \label{eq:mu}
    \end{align}
    
    % These constraints calculate the balances of hours for each resource.

    The definition of the consumption of hours for each resource at each period is forced by \ref{eq:usage2}. The monthly remaining usage and elapsed time are calculated by \ref{eq:balance1} and \ref{eq:balance2} respectively. Their initial states are governed by \ref{eq:initial} and \ref{eq:initial2} and their limits by \ref{eq:me} and \ref{eq:mu}.
    ...
    \begin{align}
        ...
        & state_{rst_2} \geq start_{rt_1} 
                & \hspace{10mm} (t_1, t_2) \in \mathcal{T}, t_1 \leq t_2 \leq t_1 + md - 1, r \in \mathcal{R}, s = \mq{M} \label{eq:start1}\\
        % if maintenance doesn't start, we cannot have maintenance:
        & \sum_{t_1 \in \mathcal{T}} start_{rt_1} \geq state_{rst_2} 
                & \hspace{10mm} t_2 \in \mathcal{T}, t_1 \leq t_2 \leq t_1 + md - 1, r \in \mathcal{R}, s = \mq{M} \label{eq:start2}\\
        & min\_avail \leq \sum_{r} state_{rst}
                & \hspace{10mm} t \in \mathcal{T}, r \in \mathcal{R}, s = \mq{A} \label{eq:available}
    \end{align}
    ...
    % \begin{align}
        ...
        % min available
        % max maintenance
        % & max\_maint \geq \sum_{r} state_{rst}
        %         & \hspace{10mm} t \in \mathcal{T}, r \in \mathcal{R}, s = \mq{M} \label{eq:maintenance}
    % \end{align}
    ...
    %     % rut initialize
    %     & rut_{rt} = rut\_ini_a
    %             & \hspace{10mm} t = 0, r \in \mathcal{R} \label{eq:remaining1}\\
    %     % ret initialize
    %     & ret_{rt} = ret\_ini_a
    %             & \hspace{10mm} t = 0, r \in \mathcal{R} \label{eq:remaining2}\\
    %     % fixed states during planning period
    %     & state_{rst} = 1
    %             & \hspace{10mm} t \in \mathcal{T}, r \in \mathcal{R}; s \in \mathcal{S}\, | if\, state\_ini_{rt} = s \label{eq:fixedstates}
    % \end{align}
