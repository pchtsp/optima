...
%  TODO!: re-arrange all this better.
...
\section{The problem}
    ...
    The following are the possible $s \in \mathcal{S}$ states for a resource to be in any given time: Assigned to a task ('V'), Receiving maintenance ('M'), Available ('N').
    ...
    These maintenances take an amount of time equal to $md_m$ periods and cannot be interrupted. The number of resources under maintenance in any given period is limited to a maximum capacity which depends on each period and is represented by $c_t$.
...
\section{The model}
    ...
    The decisions involve, basically, the management of the previously defined resources. We can summarize the main decisions the following way: if resource $a$ is in state $s$ in each period $t$ and if resource $a$ is assigned to task $v$ in period $t$.
    ...
    The limit in the number of resources assigned to task and the number of resource maintenances that can be done in any given month are modeles via \ref{eq:taskres} and \ref{eq:capacity} respectively.
    ...
    Constraint \ref{eq:state1} ties the states with the assignment to a task. Resources have only one state at each period and this is guaranteed by \ref{eq:state2}.
