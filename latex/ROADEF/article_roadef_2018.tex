...
\section{The model}
    ...
    The objective is to both minimize the maximum number of maintenances $\text{Min}\, (max\_maint)$ and maximize the minimal number of available aircraft \text{Max}\, (min\_avail).
...
\section{Conclusions}
    ...
    The present modelization is a new way to formulate the Military Flight Planning Model. Unlike previous work, it assigns aircraft to missions instead of just using an aggregate hourly demand per period. This is useful in order to improve the model by including minimum durations for mission assignment and other more complex constraints.
