...
\subsection{Objective function and constraints}
    ...
    \begin{align}
        & \text{Min}\; \sum_{t_1 \in \mathcal{T}^{M_{ini}}_i, t_2 \in \mathcal{T}^M_{t_1}, i \in \mathcal{I}} m_{it_1t_2}  \label{eq:objective1}\\
        & \text{Min}\; \sum_{t_1 \in \mathcal{T}^{M_{ini}}_i, t_2 \in \mathcal{T}^M_{t_1}, i \in \mathcal{I}} m_{it_1t_2} \times H^M - \sum_{i \in \mathcal{I}} rft_{i|\mathcal{T}|} \label{eq:objective2}
    \end{align}
...
% Constraints (\ref{eq:start3}) control the minimum duration of a consecutive mission assignment. If aircraft $i$ was firstly assigned to mission $j$ in period $t$, it has to be assigned to it during the following $t' \in \mathcal{T}_{jt}^{MT}$ periods. This is a stronger version of the constraint $a_{jt'i}^{s} \leq a_{jti}$.
    ...
    \begin{align}
       % For now, we only want one or possibly two maintenances...
        ...
        % & m_{it_1t_2} = 0
        %   & t_1 \in \mathcal{T}^{m_{ini}}_i, t_2 \in \mathcal{T}^M_{t_1} \cup \{|\mathcal{T}|\}, i \in \mathcal{I}\label{eq:rct_min_init} \\
        & m_{it_1t_2} =  0
