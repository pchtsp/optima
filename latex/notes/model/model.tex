...
\subsection{Input data}
    ...
    \subsubsection{Auxiliary sets}
        ...
        \begin{tabular}{p{15mm}p{140mm}}
            ...
            $n \in \mathcal{N}$     &  maintenance cycles for aircraft. Up to 3. \\
    ...
    \subsubsection{Fleet parameters}
        ...
        \begin{tabular}{p{15mm}p{125mm}p{15mm}}
            ...
            $H^{Clust}_{kn}$    & maximum number of mean accumulated flight time for cluster $k$ at maintenance cycle $n$. & hours. \\
    ...
    \subsubsection{Objective function weights}
        ...
        \begin{tabular}{p{15mm}p{125mm}p{15mm}}
            $PA^{Clust}_s$   & unit penalty for violating serviceability constraint in slot $s$. & \\
            $PH^{Clust}_s$   & unit penalty for violating sustainability constraint in slot $s$. & \\
        \end{tabular}
...
\subsection{Variables}
    ...
    \begin{tabular}{p{8mm}p{147mm}}
        $h_{in}$  &  Continuous, positive. Sum of the accumulated monthly consumption in flight hours for aircraft $i \in I$ for maintenance cycle $n$. \\
        $e^{AClust}_{kts}$ & Continuous, positive. Elastic variable for serviceability constraint. \\
        $e^{HClust}_{kts}$ & Continuous, positive. Elastic variable for sustainability constraint. \\
    \end{tabular}
...
\subsection{Objective function and constraints}
    ...
    \begin{align}
        & \text{Min}\; 
        \sum_{\substack{i \in \mathcal{I}, \\ 
            t_1 \in \mathcal{T}^{M_{Init}}_i, \\ 
            t_2 \in \mathcal{T}^M_{t_1} - |\mathcal{T}|}
            }  
            m_{it_1t_2} \times H^{max} 
        + \sum_{k \in \mathcal{K}, t \in \mathcal{T}, s \in \mathcal{S}} PA^{Clust}_s e^{AClust}_{kts} 
        + \sum_{k \in \mathcal{K}, n \in \mathcal{N}, s \in \mathcal{S}} PH^{Clust}_s e^{HClust}_{kns}
        \label{eq:objective1}
    \end{align}
    ...
    \begin{align}
       & \sum_{\substack{i \in \mathcal{IK}_k, \\ (t_1, t_2) \in \mathcal{T}\mathcal{T}\mathcal{T}_{t}}} m_{it_1t_2} + N^{Clust}_{kt} \leq A^{Clust}_{kt} + \sum_{s \in \mathcal{S}} e^{AClust}_{kts}
        &k \in \mathcal{K}, t \in \mathcal{T} \label{eq:serviceability-cluster}\\
       & \sum_{i \in \mathcal{IK}_k} h_{in} \leq |{IK}_k| H^{Clust}_{kn} + \sum_{s \in \mathcal{S}} e^{HClust}_{kns}
