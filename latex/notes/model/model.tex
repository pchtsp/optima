...
\subsection{Input data}
    ...
    \subsubsection{Mission parameters}
        ...
        \begin{tabular}{p{15mm}p{95mm}p{15mm}}
...
\subsection{Objective function and constraints}
    ...
    \begin{align}
        & \text{Min}\; \sum_{i \in \mathcal{I}, t_1 \in \mathcal{T}^{M_{Init}}_i, t_2 \in \mathcal{T}^M_{t_1} - |\mathcal{T}|} m_{it_1t_2}  \label{eq:objective1}\\
        % & \text{Min}\; \sum_{i \in \mathcal{I}, t_1 \in \mathcal{T}^{M_{Init}}_i, t_2 \in \mathcal{T}^M_{t_1}} m_{it_1t_2} \times H^{max} - \sum_{i \in \mathcal{I}} rft_{i|\mathcal{T}|} \label{eq:objective2}
    \end{align}
...
%                 & t =1, ..., \mathcal{T}, j \in \mathcal{JT}_t, i \in \mathcal{IJ}_j \label{eq:start1} \\
%         & a^s_{j0i} \geq a_{j0i} - 1 \!1_{i \in \mathcal{A}^{Init}_j}
%                 & j \in \mathcal{J}_0, i \in \mathcal{IJ}_j \label{eq:start2} \\
%         & \sum_{t' \in \mathcal{T}^{MT}_{jt}} a^s_{jt'i} \leq a_{jti} 
%         & j \in \mathcal{J}, t \in \mathcal{TJ}_j, i \in \mathcal{IJ}_j \label{eq:start3}
%     \end{align}
...
% Constraints (\ref{eq:start3}) control the minimum duration of a consecutive mission assignment. If aircraft $i$ was firstly assigned to mission $j$ in period $t$, it has to be assigned to it during the following $t' \in \mathcal{T}_{jt}^{MT}$ periods. This is a stronger version of the constraint $a_{jt'i}^{s} \leq a_{jti}$.
    ...
    \begin{align}
        % remaining used time
        %  & u_{it} \geq \sum_{j \in \mathcal{JT}_t \cap \mathcal{JI}_i} \sum_{(t_1, t_2) \in \mathcal{T}\mathcal{T}\mathcal{J}_{jt}} a_{ijt_1t_2} H_j 
        %     & t =1, ..., \mathcal{T}, i \in \mathcal{I} \label{eq:flight1}\\
        %  & u_{it} \in [U^{min}, \max_j{\{H_j\}}]
        %     & t =1, ..., \mathcal{T}, i \in \mathcal{I} \label{eq:flight_lower}\\
        %  & rft_{it} \leq rft_{i(t-1)} + H^{max} \sum_{(t_1, t_2) \in \mathcal{T}\mathcal{T}\mathcal{T}_{t}} m_{it_1t_2} - u_{it}
        %     & t =1, ..., \mathcal{T}, i \in \mathcal{I} \label{eq:rft_upper}\\
        % & rft_{i0} = Rft^{Init}_i
        %        & i \in \mathcal{I} \label{eq:rft_initial}\\
        % & rft_{it} \geq H^{max} \sum_{(t_1, t_2) \in \mathcal{T}\mathcal{T}\mathcal{T}_{t}} m_{it_1t_2}
