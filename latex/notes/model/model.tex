...
\section{Optimization model}
  ...
  \subsection{Sets}
    ...
    \begin{tabular}{p{5mm}p{170mm}}
        ...
        $\mathcal{I}_k$     &  resources $i \in \mathcal{I}$ that are included in cluster $k$. \\
        ...
        $\mathcal{T}^{m}_t$ &  time periods $t' \in \mathcal{T}$ such that $t' \in \{ \max{\{1, t - E^{m}+1\}},  ..., {t}\}$. \\
        $\mathcal{T}^{M}_t$ &  time periods $t' \in \mathcal{T}$ such that $t' \in \{ \max{\{1, t - E^{M}+1\}},  ..., \max{\{1, t - E^{m}+1\}}$\}. \\
  ...
  % TODO: previously assigned missions.
  % TODO: previous ret.
  % TODO: previous maintenaces.
    ...
    \begin{tabular}{p{8mm}p{167mm}}
        ...
        % $A_j$     &  resources $i \in \mathcal{I}$ that can be assigned to task $j$. \\
  ...
  \subsection{Variables}
    ...
    \begin{tabular}{p{8mm}p{167mm}}
        ...
        $a^s_{jti}$ &  =1 if resource $i$ starts a new assignment to task $j$ in period $t$. If $a_{jti} = 1$ and $a_{j(t-i)} = 0$. \\  
  ...
  \subsection{Constraints}
    ...
    where the first expression counts all the resource hours given via maintenances and the second expression measures the amount of remaining hours at all resources at the end of the planning horizon.
    ...
    % where weights $W_1$ and $W_2$ are chosen by the decision maker. $W_1$ penalizes the maximum number of resources in maintenance among all periods. $W_2$ penalizes the maximum number of unavailable resources among all periods.
    ...
    Constraints (\ref{eq:start1}) controls the definition of starting a consecutive assignment: the variable activates if a resource $i$ is assigned a task $j$ in period $t$ and was not assigned this task in the previous period ($t-1$).
    Constraints (\ref{eq:start2}) controls the minimum duration of a consecutive task assignment. If an assignment to task $j$ is started in period $t$, it needs to continue in the following $t' \in \mathcal{T}^{MT}_{jt}$ periods.
    ...
    Constraints (\ref{eq:avalaibility-cluster}) control a minimum availability for each cluster $k$ of resources. A cluster is defined by the most general group of resources that is required, exclusively, by at least one task.
    ...
    The minimum and maximum elapsed time are defined by (\ref{eq:ret_min}) and (\ref{eq:ret_max}) respectively. Constraints (\ref{eq:ret_min}) guarantee that if a maintenance is done in some period $t$, we know that it cannot be done in the following $t' \in \mathcal{T}^{m}_t$ periods. Constraints (\ref{eq:ret_max}) guarantee that, if we have a maintenance in period $t$, we have to start at least one maintenance among periods $t' \in \mathcal{T}^{M}_t}$. 
