...
\section{Results}
  ...
  \caption{Solution details on the progress of the integer solution. "id" shows the name of the instance; "first" is the first solution integer found; "sol. cuts" the solution after the cuts; "last" is the last solution found at the time limit; "cuts (\#)" the number of cuts; and "cuts (s)" the time the cuts took in seconds}
  \vspace{-0.5cm}
  \label{tab:results2}
  \end{center}
  \end{table}
  ...
  Figure \ref{fig:progress} shows an example of progress for instance with id=1817.
  ...
  \caption{Comparison of instances where variations on the weight of its two objectives has been done. Instance id=1331.}
  \vspace{-0.5cm}
  \label{tab:multiobj}
  \end{center}
  \end{table}
...
\section{Conclusions and further work}
  ...
  Regarding the multi-objective nature of the problem, the bi-objective analysis appears to indicate the existence of a small number of solutions that are Pareto optimal. If this is the case, an iterative approach that explores these solutions could be done. To confirm the true number of solutions, an analysis using epsilon constrains should follow. Further tests with different, bigger, instances could also reveal more information regarding the relation between the two objectives. Improvement in the mono-objective modeling could also lead to producing non-dominated solutions. In addition to this, new objectives could be included into the analysis, such as smoothing the remaining usage time among periods instead of doing it only in the last period.

  Another possible reason for the difficult in solving big instances could be the quantity of symmetries among candidates for each task: there exist a very big number of possible candidates to be potentially assigned for each task with respect with the actual needed resources of the task. This could be handled by assigning priorities to some combinations of resource-task or by limiting the number of candidates that can be assigned to each task by some heuristic that clusters resources with tasks.

  As new constraints are added, such as minimum durations for assignments and the possibility to "store" resources for a predefined time, the present instances will grow bigger still. This, of course, will require new better techniques to decompose the problem into smaller problems using heuristics or take a decomposition approach such as Column Generation.
