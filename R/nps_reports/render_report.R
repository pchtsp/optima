...
options <- list(
    list('size'='get_all_fixLP', 'num_tasks'=2, 'scale'='log2')
    # ,list('size'='get_all_stoch', 'num_tasks'=3, 'scale'='identity')
...
for (option in options){
    rmarkdown::render("nps_reports/results.Rmd", 
                      output_file = sprintf('nps_reports/NPS_report_%s_%s_%s.%s', 
                                            option$size, option$num_tasks, option$scale, format),
                      params = option, 
