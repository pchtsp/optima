...
The variance in the frequency of maintenances is a side objective that was not been taken into account previously. To calculate it, we have, for each fleet type, measured the variance of the distance between maintenances. And we have summed all variances for all fleet types. We do this analysis for all integer solutions obtained.
As can be seen, in almost all of the cases there is a reduction in the variance. The mean relative difference is `r mean_var_reduction` and 95\% of instances have less than `r quant95_var_reduction`.
