...
The following boxplot shows the distribution of the gap from the models with respect to the `base` model. Results are shown in logaritmic scale and so we have replace all negative values for a very small one.

```{r}
ggplot(data=quality_degr_all_filt, aes(x=experiment, y=dif_perc)) + theme_minimal() + geom_boxplot() + xlab('Experiment') + ylab('% difference in objective (log2 scale)') + theme(text = element_text(size=20)) + coord_flip() + scale_y_continuous(trans='log2', labels=scaleFUN)
...
The following graph shows the distribution of relative gaps between the best integer solutions found in each of the two models. A negative relative gap means the model outperformed the `base` model in that particular instance. Results are shown in logaritmic scale and so we have replace all negative values for a very small one.

```{r quality_perf_plot}

ggplot(data=quality_perf_all_filt, aes(x=experiment, y=dif_perc)) + theme_minimal() + geom_boxplot() + xlab('Experiment') + ylab('% difference in objective (log2 scale)') + theme(text = element_text(size=20)) + coord_flip() + scale_y_continuous(trans='log2', labels=scaleFUN)
...
Regarding the accomplishment of soft constraints, we can analyze for mutually **optimal solutions**, the quantity of violations in their solutions. For each model and instance, we subtract the `base` model errors. We show only the extra errors (in number) compared to the `base` case.
...
And the complete distributions in logaritmic scale:

```{r soft_constraints_plot}

ggplot(data=errors_all, aes(x=experiment, y=dif)) + theme_minimal() + geom_boxplot() + xlab('Experiment') + ylab('absolute difference in errors  (log2 scale)') + theme(text = element_text(size=20)) + coord_flip() + scale_y_continuous(trans='log2', labels=scaleFUN)
...
To better visualize the results, we use a log 2 scale.

```{r graph_variances}

# qplot(var_per, xlab='')

ggplot(data=variances_all_filt, aes(x=experiment, y=dif_perc)) + theme_minimal() + geom_boxplot() + xlab('Experiment') + ylab('Difference in variance (in % of base case) (log2 scale)') + theme(text = element_text(size=20)) + coord_flip() + scale_y_continuous(trans='log2', labels=scaleFUN)
