...
* Each mission requires a certain number $r_{j}$ of aircraft $i \in \mathcal{I}$ which it employs for $h_j$ hours in each period. An aircraft assigned to a mission $j$ must be assigned for at least $MT^{min}_{j}$ and at most $MT^{max}_{j}$ consecutive periods.
* The heterogeneous fleet requires recurrent preventive maintenance operations (checks) as a result of the total flight hours since the last check. There are also min (max) distances between maintenances.
* A check takes exactly $m$ periods and cannot be interrupted. There is a limited capacity for maintenances at each period.
...
\begin{align}
& \sum_{(j, t, t') \in \mathcal{J}\mathcal{T}\mathcal{T}_{ic}} a_{ijtt'} H^\prime_{jtt'} + U^{\prime}_{tc} \leq H^{max} + H^{max} (1 - m_{ip}) & \notag \\
...
.pull-left[
* The distance between maintenance has a maximum of $E^{max}$ periods.
...
2. Use forecasting model to predict bounds on distance between maintenaances: $\hat{\mu}_{t'-t}^{lb}$,  $\hat{\mu}_{t'-t}^{ub}$.
