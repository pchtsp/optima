...
class DAG(object):
    ...
    def __init__(self, instance, resource, nodes_ady=None, get_artificial=True):
    ...
    def get_all_patterns(self, resource):
        source = nd.get_source_node(self.instance, resource)
        sink = self.sink
        return self.nodes_to_patterns(self.g, self.refs, self.refs_inv, source, sink)
...
def get_artificial_nodes(nodes_ady):
    return \
        nodes_ady.keys_tl().\
